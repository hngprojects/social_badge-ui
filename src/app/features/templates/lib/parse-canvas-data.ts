...
import { BACKGROUND_IMAGE_BY_PALETTE, EDITOR_PALETTES } from "./palette-mapping";

function findStatic(fields: CanvasData["fields"], key: "event_name" | "event_date"): CanvasStaticField | undefined {
  return fields.find(
...
function parseEventDateParts(value: string): { eventDate: string; eventTime: string } {
  if (value.includes(" · ")) {
...
function backgroundToEditorState(canvas: CanvasData): Pick<
  CustomizeEditorState,
  "bgMode" | "paletteId" | "gradientColors" | "gradientDirection" | "solidColor" | "backgroundImageUrl"
> {
  ...
  if (bg.type === "solid") {
    const match = EDITOR_PALETTES.find((p) => p.from.toLowerCase() === bg.color.toLowerCase());
    return {
      bgMode: "solid",
...
export function parseCanvasDataToEditorState(
  ...
  canvas: CanvasData,
  meta?: Partial<Pick<OrganiserTemplateResponse, "title" | "default_caption" | "destination_link" | "hashtags" | "access_type">>,
): CustomizeEditorState {
  const layoutId = canvas.layout_id;
  const eventNameField = findStatic(canvas.fields, CANVAS_FIELD_KEYS.EVENT_NAME);
  const eventDateField = findStatic(canvas.fields, CANVAS_FIELD_KEYS.EVENT_DATE);
  const participantName = findParticipantInput(canvas.fields, CANVAS_FIELD_KEYS.PARTICIPANT_NAME);
  const roleTitle = findParticipantInput(canvas.fields, CANVAS_FIELD_KEYS.ROLE_TITLE);
  const hasPhoto = canvas.fields.some((f) => f.key === CANVAS_FIELD_KEYS.PARTICIPANT_PHOTO);
...
export function createDefaultEditorState(platformTemplateId: string): CustomizeEditorState {
  ...
  const fromPalette = EDITOR_PALETTES.find((p) => p.id === palette) ?? EDITOR_PALETTES[0];
  ...
  return {
    ...
    fontId: layoutId === "speaker_card_v1" ? "fraunces" : "inter",
    titleSize: "MEDIUM",
    defaultCaption: "",
