...
export default function Testimonials({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrent((prev) =>
        prev === Math.ceil(testimonials.length / slidesPerView) - 1
          ? 0
          : prev + 1,
      );
    }, 6000);
      ...
      <div className="max-w-360 px-4 md:px-10 lg:px-30 mx-auto">
        ...
        <div className="mb-16">
          ...
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-sans font-semibold  text-[#3d3d3d]">
            Organizers who{" "}
            <span className="italic text-primary font-fraunces">get it.</span>
          </h2>
        </div>
        ...
        <div className="relative ">
          <div className="overflow-y-auto overflow-x-hidden scrollbar-hide md:overflow-hidden max-h-150 md:max-h-none">
            ...
            >
              {testimonials.map((testimonial, index) => (
                <div
                  key={testimonial.id}
                  className="w-full md:w-[calc((100%-1.5rem)/2)] p-1 shrink-0 h-auto md:h-75"
                >
                  <div
                    className={`${getCardColor(index)} rounded-[14px] p-8 transition-shadow duration-300`}
                  >
                    {/* Quote */}
                    <p className="text-base md:text-[18px] lg:text-[20px] text-[#3d3d3d] leading-[30px] mb-16 font-sans">
                      {testimonial.quote}
                    </p>
          ...
          <div className="hidden md:flex items-center justify-center mt-10">
            ...
            <div className="flex gap-2">
              {Array.from({
                length: Math.ceil(testimonials.length / slidesPerView),
              }).map((_, index) => (
                <button
                  key={index}
